...
import com.ccsw.tutorial.client.ClientClient;
import com.ccsw.tutorial.client.model.ClientDto;
import com.ccsw.tutorial.game.GameClient;
...
public class LoanController {
    ...
    public Page<LoanDto> find(@RequestBody LoanSearchDto dto) {
        ...
        return new PageImpl<>(page.getContent().stream().map(loan -> {
            ...
            loanDto.setGame(games.stream().filter(g -> g.getId().equals(loan.getIdGame())).findFirst().orElse(null));
            loanDto.setClient(clients.stream().filter(c -> c.getId().equals(loan.getIdClient())).findFirst().orElse(null));
